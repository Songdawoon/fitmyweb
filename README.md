...
**확정 전까지는 몇 번이든 고칠 수 있습니다.** 자료를 나중에 찾아 채우는 경우가
흔합니다. 고객이 수정하면 `seen_at` 을 비워 관리자 화면에 다시 `New` 로 뜹니다 —
고쳤는데 아무도 모르면 고친 의미가 없습니다.

### 확정하면 잠긴다

제작을 시작한 뒤에 내용이 바뀌면 무엇을 기준으로 만든 건지 알 수 없게 됩니다.
관리자 상세의 **"확정하고 제작 시작"** 을 누르면 `locked_at` 이 찍히고 고객은 더
이상 고칠 수 없습니다.

잠긴 뒤에도 **고객은 자기가 보낸 내용을 계속 볼 수 있고 수정만 막힙니다** — 링크를
통째로 죽이면 "내가 뭐라고 썼더라" 를 묻는 연락이 늘어납니다.

막는 지점은 두 곳입니다. 라우트가 409 로 이유를 알려 주고, `saveBrief` 는
`WHERE locked_at IS NULL` 로 DB 가 막습니다 — 라우트에서 확인한 뒤 저장하기 전
사이에 관리자가 확정할 수 있기 때문입니다.

**"수정 다시 허용"** 으로 되돌릴 수 있습니다. 실수로 확정한 경우와, 제작 도중
고객에게 보완을 요청해야 하는 경우가 실제로 생깁니다. 작성 전인 건은 확정할 수
없습니다 — 빈 내용으로 잠기면 고객이 아무것도 못 적습니다.
