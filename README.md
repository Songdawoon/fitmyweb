...
- `coupons` — 회원가입 쿠폰. `(user_id, kind)` 유니크로 1인 1장을 보장합니다.
  코드는 `MFW-XXXXXXXX` 형식이고, 발급·사용 흐름은 아래 "쿠폰" 절 참고.
...
**발급** — 홈 팝업의 "쿠폰 다운받기" 버튼 하나로만 발급됩니다. 로그인 여부는
서버(`POST /api/coupon`)가 판단해 비로그인이면 401 을 주고, 화면이
`/login?callbackUrl=/?coupon=1` 로 보냅니다. 로그인 후 돌아오면 팝업이 즉시
열리며 발급이 자동으로 이어집니다.

계정당 1장이라는 규칙은 앱 로직이 아니라 `(user_id, kind)` 유니크 인덱스가
보증합니다. `issueSignupCoupon` 은 조회 없이 바로 insert 를 시도하고, 충돌이
나면 기존 쿠폰을 읽어 "이미 받으셨어요" 로 답합니다 — 버튼을 연타해도 두 장이
나올 수 없습니다. 로그인만으로는 발급되지 않습니다(그러면 버튼이 언제나
"이미 발급됨" 이 되기 때문).

**사용** — 결제 화면이 세션으로 미사용 쿠폰을 조회해 기본 적용하고, 코드를
`custom_data.couponCode` 에 실어 보냅니다. 서버는 이 코드로 DB 를 다시 읽어
미사용 쿠폰인지 확인한 뒤 **쿠폰에 적힌 금액만** 할인으로 인정하고
`plan.price - discount` 와 실제 결제액을 대조합니다(`lib/portone.ts`). 그래서
클라이언트가 금액이나 할인액을 조작해 보내도 통과하지 못합니다. 결제가
확정되면 `used_at` 이 찍히고, 완료 라우트와 웹훅이 같은 건을 두 번 보고해도
미사용일 때만 갱신하므로 멱등합니다.
